...
from .models import SubscriptionPlan, Subscription, SubscriptionHistory, PinnedPost

@admin.register(SubscriptionPlan)
class SubscriptionPlanAdmin(admin.ModelAdmin):
    list_display = (
        'name', 'price', 'duration_days', 'subscriptions_count',
...
class SubscriptionAdmin(admin.ModelAdmin):
    ...
    def days_remaining_display(self, obj):
        """Отображение оставшихся дней до конца подписки"""
    ...
    def activate_subscriptions(self, request, queryset):
        ...
        for subscription in queryset:
            if subscription.status != 'active':
                ...
                count += 1
        self.message_user(request, f'{count} subscriptions have been activated.')
    ...
    def cancel_subscriptions(self, request, queryset):
        ...
        for subscription in queryset:
            if subscription.status == 'active':
                subscription.cancel()
                count += 1
            self.message_user(request, f'{count} subscriptions have been cancelled')
    ...
    def expire_subscriptions(self, request, queryset):
        ...
        for subscription in queryset:
            if subscription.status == 'active':
                subscription.expire()
                count += 1
            self.message_user(request, f'{count} subscriptions have been expired')
...
class PinnedPostAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        """Запрещаем на создание через админку"""
        return False
...
class SubscriptionHistoryAdmin(admin.ModelAdmin):
    ...
    def subscription_link(self, obj):
        ...
        return format_html('<a href="{}">{} - {}</>, url, obj.subscription.user.username, obj.subscription.plan.name')
